...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations)
{
    ...
    // NOTE: This method will NOT be called by the grading code. But we will probably find it useful to
                                   ...
                                   const Map& map_landmarks)
{
    ...
    for (int i = 0; i < num_particles; i++)
    {
        ...
        for (unsigned int j = 0; j < mappedObservations.size(); j++)
        {
            ...
            double weight = (1 / (2 * M_PI * stdLandmarkRange*stdLandmarkBearing)) *
                            exp(-(dX * dX/(2 * stdLandmarkRange*stdLandmarkRange) + (dY * dY/(2 * stdLandmarkBearing*stdLandmarkBearing))));
            if (weight == 0)
            {
                particles[i].weight *= tolerance;
            }
            else
            {
